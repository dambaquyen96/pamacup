...
void BFS(data x)
{
	...
	while (!q.empty())
	{
		...
		for (int i=0; i<4; i++)
		{
			int rm = f.r+y_xq[i];
			int cm = f.c+x_xq[i];
			if (inside(rm, cm)==1 && check[rm][cm]==0 && map[rm][cm]!='#' && isMa(rm, cm)==0 && isRoMa(rm, cm)==0)
			{
				data tmp;
				tmp.r = rm;
				tmp.c = cm;
...
//	while (1);
	...
	int x = rand()%10;
	data pacman1;
    if (x<3) pacman1 = truyvet(blinky);
    else pacman1 = truyvet(tmp);
    if (pacman1.r==0 && pacman1.c==0) pacman1 = Random(pacman);
    pacman = pacman1;
